...
def compute_adaptive_config(metric_data, metric_column, default_config=None):
    ...
    if metric_column not in metric_data.columns or len(metric_data) < 5:
        print(f"Warning: Insufficient data for {metric_column}, using default")
        return default_config
    
    values = metric_data[metric_column].dropna()
    
    # 1. 计算 k_thr：适配 TT 数据集的高波动
    ...
    if 'TimeStamp' in metric_data.columns:
        ...
        if len(timestamps) > 1:
            time_diff = (timestamps.max() - timestamps.min()) / (len(timestamps) - 1) * 1000  # 转换为毫秒
    ...
    print(f"Adaptive config for {metric_column}: k_thr={k_thr}, std_thr={std_thr}, win_size={win_size}")
    return config
...
class MetricEvent:
    ...
    def read(self, metric):
        ...
        try:
            ...
            if self.debug and self.debug_count < 5:
                ...
                if 'timestamp' in data.columns:
                    ...
                    print(f"Readable timestamp range (UTC): "
                          f"{datetime.fromtimestamp(data['timestamp'].min()/1000, pytz.UTC)} to "
                          f"{datetime.fromtimestamp(data['timestamp'].max()/1000, pytz.UTC)}")
                    print(f"Readable timestamp range (UTC+8): "
                          f"{datetime.fromtimestamp(data['timestamp'].min()/1000, tz)} to "
                          f"{datetime.fromtimestamp(data['timestamp'].max()/1000, tz)}")
                for col in ['SuccessRate(%)', 'LatencyP50(s)', 'LatencyP90(s)', 'LatencyP95(s)', 'LatencyP99(s)']:
                    if col in data.columns:
                        print(f"{col} stats: min={data[col].min():.4f}, max={data[col].max():.4f}")
                        print(f"{col} mean={data[col].mean():.4f}, std={data[col].std():.4f}")
                        value_range = data[col].max() - data[col].min()
                        value_cv = data[col].std() / data[col].mean() if data[col].mean() != 0 else 0
                        print(f"{col} range: {value_range:.4f}, CV: {value_cv:.4f}")
                if 'ServiceName' in data.columns:
                    print(f"Services: {data['ServiceName'].nunique()}, sample: {list(data['ServiceName'].unique()[:3])}")
                self.debug_count += 1
            ...
            if self.adaptive_config:
                for col in ['SuccessRate(%)', 'LatencyP50(s)', 'LatencyP90(s)', 'LatencyP95(s)', 'LatencyP99(s)']:
                    if col in data.columns:
                        self.metric_configs[f"{metric}_{col}"] = compute_adaptive_config(data, col)
    ...
    def detect_anomalies_for_services(self, metric_data, start_ts, end_ts, kpi_name, case_id, metric=None, stats=None):
        ...
        if 'ServiceName' not in metric_data.columns:
            if self.detailed_debug:
                print(f"  No ServiceName column in data")
            return anomalies
        ...
        metric_columns = ['SuccessRate(%)', 'LatencyP50(s)', 'LatencyP90(s)', 'LatencyP95(s)', 'LatencyP99(s)']
        for metric_col in metric_columns:
            if metric_col not in metric_data.columns:
                continue
            
            if self.adaptive_config and f"{metric}_{metric_col}" in self.metric_configs:
                detector = Ksigma(self.metric_configs[f"{metric}_{metric_col}"])
            else:
                detector = self.detector
            
            for service_name in metric_data['ServiceName'].unique():
                services_processed += 1
                service_data = metric_data[metric_data['ServiceName'] == service_name].copy()
                
                if len(service_data) < 3:
                    if self.detailed_debug:
                        print(f"    Service {service_name}, {metric_col}: insufficient data ({len(service_data)} points)")
                    continue
                
                services_with_enough_data += 1
                
                try:
                    values = service_data[metric_col]
                    value_mean = values.mean()
                    value_std = values.std()
                    value_cv = value_std / value_mean if value_mean != 0 else 0
                    if self.detailed_debug:
                        print(f"    Service {service_name}, {metric_col}: {len(service_data)} points, "
                              f"mean={value_mean:.4f}, std={value_std:.4f}, cv={value_cv:.4f}")
                
                    res = detector.detection(service_data, metric_col, start_ts/1000, end_ts/1000)
                    
                    if res[0] is True and res[2] > 0.3:
                        services_with_anomalies += 1
                        anomaly_ts_ms = int(res[1] * 1000) if res[1] else None
                        anomalies.append([anomaly_ts_ms, service_name, f"{kpi_name}_{metric_col}", res[2]])
                        if self.debug:
                            print(f"    ANOMALY! Service: {service_name}, KPI: {kpi_name}_{metric_col}, Score: {res[2]:.4f}")
                    elif self.detailed_debug:
                        print(f"    Service {service_name}, {metric_col}: no anomaly detected, score: {res[2]:.4f}")
                            
                except Exception as e:
                    stats['detection_failures'] += 1
                    if self.debug:
                        print(f"    Error detecting anomaly for service {service_name}, {metric_col}: {e}")
                    continue
                ...
                try:
                    ...
                    if len(window_data) > 0:
                        ...
                        if self.detailed_debug and case_id <= 5:
                            ...
                            if 'ServiceName' in window_data.columns:
                                print(f"  Services in window: {window_data['ServiceName'].nunique()}")
                                print(f"  Sample ServiceName: {list(window_data['ServiceName'].unique()[:3])}")
                    ...
                    if len(window_data_offset) > 0:
                        ...
                        if self.detailed_debug and case_id <= 5:
                            ...
                            if 'ServiceName' in window_data_offset.columns:
                                print(f"  Services in window: {window_data_offset['ServiceName'].nunique()}")
                                print(f"  Sample ServiceName: {list(window_data_offset['ServiceName'].unique()[:3])}")
                        anomalies = self.detect_anomalies_for_services(
...
def run_detection_with_config(labels, data_base_dir, detector_config, config_name, adaptive_config=False, save_path_base=None):
    ...
    save_path_original = f'{save_path_base}_original.json' if save_path_base else f'/home/fuxian/lixuyang/DiagFusion25+tt/liningshuai/metric_events_tt_{config_name}_original.json'
    save_path_offset = f'{save_path_base}_offset_8h.json' if save_path_base else f'/home/fuxian/lixuyang/DiagFusion25+tt/liningshuai/metric_events_tt_{config_name}_offset_8h.json'
        ...
        for case_id, events in res.items():
            if len(events) > 0:
                ...
                for event in events[:3]:
                    if len(event) >= 4:
                        timestamp, service_name, kpi_name, score = event
                        readable_time = ts_to_date(timestamp/1000) if timestamp else "Unknown"
                        print(f"  - {readable_time}: {service_name} - {kpi_name} (score: {score:.3f})")
                if len(events) > 3:
...
if __name__ == '__main__':
    data_base_dir = '/home/fuxian/DataSet/tt'  # Updated to correct path
    tt_days = ['2023-01-29', '2023-01-30']
    ...
    for config_name in configs.keys():
        print(f"  - /home/fuxian/lixuyang/DiagFusion25+tt/liningshuai/metric_events_tt_{config_name}_original.json")
        print(f"  - /home/fuxian/lixuyang/DiagFusion25+tt/liningshuai/metric_events_tt_{config_name}_offset_8h.json")