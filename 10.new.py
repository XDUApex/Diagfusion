...
class TraceUtils:
    ...
    def getPairs(self):
        ...
        for day_str in tt_days:
            trace_dir = os.path.join(self.data_dir, f'2023-01-{day_str}', 'trace')
            ...
            for filename in os.listdir(trace_dir):
                if filename.endswith('.csv') and '_' in filename:
                    ...
                    # Extract service call pairs through ParentID and SpanID matching
                    cdata = df[['ParentID', 'PodName']].rename(columns={'ParentID': 'SpanID', 'PodName': 'cPodName'})
                    merged = pd.merge(df, cdata, on='SpanID', how='left')
    ...
    def get_trace_by_day(self, date_str: str):
        trace_dir = os.path.join(self.data_dir, f'2023-01-{date_str}', 'trace')
        if not os.path.exists(trace_dir):
            print(f"Trace directory not found: {trace_dir}")
            return pd.DataFrame()
        ...
        for filename in os.listdir(trace_dir):
            if filename.endswith('.csv') and '_' in filename:
                ...
                try:
                    ...
                    df['start_timestamp_ms'] = df['StartTimeUnixNano'] / 1e6
                    df['end_timestamp_ms'] = df['EndTimeUnixNano'] / 1e6
                    df['lagency'] = df['Duration'] / 1e6  # Convert Duration to milliseconds
                    all_traces = pd.concat([all_traces, df], ignore_index=True)
                except Exception as e:
                    print(f"Error reading {filepath}: {e}")
                    continue
        
        if all_traces.empty:
            print(f"No trace data found for day 2023-01-{date_str}")
            return pd.DataFrame()
    ...
    def data_process(self, date_str: str):
        ...
        if data.empty:
            return pd.DataFrame()
        cdata = data[['ParentID', 'PodName']].rename(columns={'ParentID': 'SpanID', 'PodName': 'cPodName'})
        return pd.merge(data, cdata, on='SpanID')
    ...
    def turn_to_timeseries(self, date_str, savepath):
        ...
        if df.empty:
            ...
            return
        df['start_timestamp_ms'] = df['start_timestamp_ms'].apply(lambda x: int(x))  # Ensure timestamp is integer
        
        # Infer status based on message field (assuming OperationName as message proxy)
        print(f"No status_code column found for day {date_str}, inferring status from OperationName field")
        df['mapped_status'] = df['OperationName'].apply(self.infer_status_from_message)
        ...
        date = ts_to_date(df['start_timestamp_ms'].iloc[0]).split()[0] if not df.empty else f'2023-01-{date_str}'
        ...
        for caller, callee in self.pairs:
            temp = df.loc[(df['PodName'] == caller) & (df['cPodName'] == callee)]
            if temp.empty:
            ...
            for k in range(points_count):
                chosen = temp.loc[(temp['start_timestamp_ms'] >= start_ts + k * delta) &
                                  (temp['start_timestamp_ms'] < start_ts + (k + 1) * delta)]
    ...
    def getPairs(self):
        if self.dataset == 'gaia':
            ...
            if pairs:
                ...
                original_data_dir = '/home/fuxian/DataSet/tt'  # Updated TT dataset path
                for day_str in tt_days:
                    trace_dir = os.path.join(original_data_dir, f'2023-01-{day_str}', 'trace')
                    if not os.path.exists(trace_dir):
                        continue
                    for filename in os.listdir(trace_dir):
                        if filename.endswith('.csv') and '_' in filename:
...
if __name__ == '__main__':
    ...
    labels = []
    data_base_dir = '/home/fuxian/DataSet/tt'  # Updated TT dataset path
    ...
    # Set save paths
    save_path_original = os.path.abspath('/home/fuxian/lixuyang/DiagFusion25+tt/liningshuai/trace_events_tt_original.json')
    save_path_offset = os.path.abspath('/home/fuxian/lixuyang/DiagFusion25+tt/liningshuai/trace_events_tt_offset_8h.json')
